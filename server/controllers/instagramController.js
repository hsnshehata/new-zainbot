...
const verifyWebhook = (req, res) => {
  ...
  const VERIFY_TOKEN = 'hassanshehata'; // Hardcodeding this to match Facebook and WhatsApp webhooks in the system
...
const handleMessage = async (req, res) => {
  try {
    ...
    for (const entry of body.entry) {
      ...
      // Instagram Webhooks can be tricky: 
      // Sometimes pageId (entry.id) is the Instagram Account ID, sometimes it's the Facebook Page ID.
      // The actual Instagram recipient ID is often inside entry.messaging[0].recipient.id
      // We will try finding the bot by pageId first. If not found, we look deeper.
      let bot = await Bot.findOne({ instagramPageId: pageId });

      if (!bot && entry.messaging && entry.messaging.length > 0) {
        const recipientId = entry.messaging[0].recipient?.id;
        if (recipientId) {
          bot = await Bot.findOne({ instagramPageId: recipientId });
          if (bot) {
            logger.info('ig_bot_found_via_recipient_id', { pageId, recipientId });
          }
        }
      }

      if (!bot) {
        logger.warn('ig_bot_not_found', { pageId, recipientId: entry.messaging?.[0]?.recipient?.id });
        continue;
      }
      ...
      if (entry.messaging) {
        for (const event of entry.messaging) {
          ...
          if (event.delivery || event.read) {
            // صمت إيصالات القراءة والاستلام لتجنب الإزعاج في السجلات
            continue;
          }
